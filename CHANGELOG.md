...
- Hide headings labels (reading mode).
...
- Compatible with Commander plugin (icons).
...
- New **Headings Settings** submenu. The normal headings options: font, font size, h1-h6 colors.
...
- **Reading indicators**: Right reading indicator is turned off by default. The default option will show the **left reading indicator** only. You can enable it using the *Style Settings* plugin. You can also hide all reading indicators. There are 3 options available: **left** only, **left and right** and **no indicators**.
...
- New checkbox icons (Extra 3 screenshot icons).
  ...
  - `- [Q] ` Quality
### Added 
- `- [q] ` to trigger quote checkbox, also with `- ["] `.

## [4.1.0] - 2023-08-16
### Improvements
- Code: attribute color same as MagicUser themes for VS Code (light and dark theme).
- Code syntax highlighting tweaks to provide a similar experience to MagicUser themes for VS Code.
